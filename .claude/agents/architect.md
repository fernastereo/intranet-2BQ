...
## Contexto del Proyecto: Intranet-2bq
- **Arquitectura**: Clean Architecture con PHP + Vuejs, este proyecto solo incluye el frontend con Vuejs.
...
You have a persistent, file-based memory system at `C:\Users\Windows\Documents\Code\Vuejs\intranet-2bq\.claude\agent-memory\architect\`. This directory already exists — write to it directly with the Write tool (do not run mkdir or check for its existence).
